  ...
  $analyzeBtn.click(function() {
    ...
    $.post('/tone', {'text': text })
      .done(function(response) {
        ...
        var cheerfulness = jsonArray.children[0].children[0].normalized_score;
        var negative = jsonArray.children[0].children[1].normalized_score;
        var anger = jsonArray.children[0].children[2].normalized_score;
        var analytical = jsonArray.children[1].children[0].normalized_score;
        var confident = jsonArray.children[1].children[1].normalized_score;
        var tentative = jsonArray.children[1].children[2].normalized_score;
        var openness = jsonArray.children[2].children[0].normalized_score;
        var agreeableness = jsonArray.children[2].children[1].normalized_score;
        var conscientiousness = jsonArray.children[2].children[2].normalized_score;
        
       	var confidenceper = Math.round(confident * 100);
       	var opennessper = Math.round(openness * 100);
       	var negativeper = Math.round(negative * 100);
        console.log(openness);
        var score = 0;
        var message = '';
        var percentc = confident/.8 * 100;
        
        
        
        score = 50*confident + 30*agreeableness; 
        if (agreeableness + confident <= 130) {
        	score -= 20;
        	if (negative < 10) {
        		score += 40;
        	}
        }
        score += 10*analytical + 5*openness + (-5)*negative + 7*cheerfulness + 9*conscientiousness
        if (negative<20 && openness>60){
        	score += 30
        }
        
		
		if (score >= 100) {
			message = 'Good job! You have a good chance of getting into most colleges.';
		} else if (score < 100 && score > 25) {
			message = 'Your essay is good, but you should still work on: confidence';
		} else if (score <= 25) {
			message = 'It could use some improvement. Areas you could improve on: agreeableness, confidence';
		}
		var scoretime = Math.round(score, 2);
		
		console.log(score);
		$("#yourscore").html(
		
		"<h2> Overall Score: "+scoretime+"</h2><p> "+message+"<p> Confidence Level: "+confidenceper+"%<p> Openness Level: "+opennessper+"%<p> Negativity Level: "+negativeper+"%");
